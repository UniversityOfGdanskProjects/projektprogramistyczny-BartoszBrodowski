...
exports.statsForMovie = async (movieId) => {
	try {
		const movie = await session.run(`MATCH (m:Movie { id: '${movieId}' }) RETURN m`);
		if (movie.records.length !== 0) {
			const result = await session.run(
				`MATCH (m:Movie { id: $movieId })<-[a:ACTED_IN]-(act:Person), (m)<-[d:DIRECTED]-(dir:Person)
				WITH m, min(act.born) AS oActorAge, max(act.born) AS yActorAge, min(dir.born) AS yDirectorAge, max(dir.born) AS oDirectorAge, max(act.popularity) AS yActorPop, avg(act.popularity) AS avgActorPop, apoc.agg.median(act.born) AS medianActorYearBorn
				MATCH (yAct:Person)-[:ACTED_IN]->(m) WHERE yAct.born = yActorAge
				WITH m, oActorAge, yActorAge, yAct, yDirectorAge, oDirectorAge, yActorPop, round(avgActorPop, 2) as avgActorPop, medianActorYearBorn
				OPTIONAL MATCH (m)<-[c:COMMENTED]-(u:User)
				WITH m, oActorAge, yActorAge, yAct, yDirectorAge, oDirectorAge, yActorPop, count(c) AS comments, avgActorPop, medianActorYearBorn
				MATCH (oAct:Person)-[:ACTED_IN]->(m) WHERE oAct.born = oActorAge
				WITH m, oActorAge, yActorAge, yAct, oAct, yDirectorAge, oDirectorAge, yActorPop, comments, avgActorPop, medianActorYearBorn
				MATCH (yDir:Person)-[:DIRECTED]->(m) WHERE yDir.born = yDirectorAge
				WITH m, oActorAge, yActorAge, yAct, oAct, yDirectorAge, oDirectorAge, yDir, yActorPop, comments, avgActorPop, medianActorYearBorn
				MATCH (oDir:Person)-[:DIRECTED]->(m) WHERE oDir.born = oDirectorAge
				WITH m, oActorAge, yActorAge, yAct, oAct, yDirectorAge, oDirectorAge, yDir, oDir, yActorPop, comments, avgActorPop, medianActorYearBorn
				MATCH (popularActor:Person)-[:ACTED_IN]->(m) WHERE popularActor.popularity = yActorPop
				WITH m, oActorAge, yActorAge, yAct, oAct, yDirectorAge, oDirectorAge, yDir, oDir, yActorPop, comments, popularActor, avgActorPop, medianActorYearBorn
				MATCH (m)<-[r:RATED]-(u:User)
				WITH m, oActorAge, yActorAge, yAct, oAct, yDirectorAge, oDirectorAge, yDir, oDir, yActorPop, comments, avg(r.rating) AS avgRating, popularActor, avgActorPop, medianActorYearBorn
				RETURN m, oActorAge, yActorAge, yAct.name as youngestActor, oAct.name as oldestActor, yDirectorAge, oDirectorAge, yDir.name as youngestDirector, oDir.name as oldestDirector, popularActor.name as mostPopularActor, yActorPop as maxActorPopularity, comments, avgRating, avgActorPop, medianActorYearBorn`,
				{ movieId }
			);
			// return result;
			return result.records.map((record) => ({
				youngestActor:
					record.get('youngestActor') + ', born ' + record.get('yActorAge')['low'],
				...
				medianActorYearBorn: record.get('medianActorYearBorn'),
			}))[0];
		} else {
